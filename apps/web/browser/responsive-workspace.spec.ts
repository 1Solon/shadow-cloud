...
for (const [layout, widths] of [
  ["mobile", [400, 639, 640, 767]],
  ["desktop", [768, 1279, 1280, 1440]],
] as const) {
  test(`campaign actions and configuration remain reachable on ${layout}`, async ({
    page,
    context,
    campaign,
  }, testInfo) => {
    ...
    await expect(
      page.getByText("No campaigns match your search"),
    ).toBeVisible();
